...
- **Dip-bounce with DCA (`scripts/dip_dca_research.py`, 2026-09-26): no edge at either frequency. PARKED.** If revived, change the ENTRY: the exits were tuned three ways and the entry is where it loses. Two untested ideas, each one fixed-rule trial against the same controls: (a) enter on a bullish break of structure after the dip, not on the dip itself; (b) take dips only during a market-wide flush, which is the move Grid profits from. A behavioural capitulation-reversal bet: buy a dip, DCA up to 3 adds, take profit on the bounce with one tranche or exit at break-even once averaged in, stop one step below the last add, 10 slots, maker limits. Two variants, each fixed before its first run:
  - **std** (1h bars): 15% below the 7d high, adds every 5%, TP +5% / BE +0.5%, stop P0×0.80, 30d max hold. **OosCoins −19.0%/yr (t −2.0), BacktestCoins −12.9%/yr (t −1.4)**, 93% wins.
  - **hf** (15m bars from the C# cache, `--variant hf`): 3% below the 24h high, adds every 1%, TP +1% / BE +0.2%, stop P0×0.96, 24h max hold. **−272%/yr and −259%/yr of capital** (additive; compounded it is ruin within months). ~19k trades/yr, costs alone −56%/yr, and it loses gross too (price −216%/yr). 89% wins at a mean of −0.14% of the slot per trade.
  - **hf-bos** (`--variant hf-bos`): after the third add the fixed stop gives way to structure. Exit on a close below the latest confirmed 15m swing low (fractal, 2 bars each side) that formed after that add. Catastrophic stop one step lower until one exists; otherwise hold for break-even. The selftest pins it on a deterministic path. **−257%/yr (OosCoins) and −238%/yr (BacktestCoins)**, vs −272 / −259 without it. Stop-outs fall from 10.9% to 5.6% of trades plus 3.3% structure exits, and wins rise to 91%. Still 0% vs random entries and 100% vs shuffled bars, so the loss sits in the entries, not in how the tail is managed.
  - **Controls:** std's 15% trigger beats random entries (70% / 100%) but not shuffled bars (20% / 50%), so there is no reversal to harvest. hf is the reverse: it beats every shuffled-bar control, so 15m prices *do* revert more than chance. But it loses to every random-entry control, so a 3%-in-24h dip selects *continuations*: the dip trigger picks the wrong moments. Every scaled variant loses; the no-stop variant shows the martingale tail (worst trade −55% of the slot).
  - **Fill-model lessons, both in `selftest`:** (1) touch-fills are optimistic. A limit at the current price "fills" whenever the next bar opens there, including when price then runs away. Limits must trade THROUGH by `THROUGH = 5bp`. Correcting this cost std ~5.5 pts/yr. (2) Adds that the fill bar's low went through after the entry must fill in that bar; skipping them keeps the one-tranche +TP target on a path that had already averaged down. (3) Compare bar vs exact fill order with the bar side charged its modelled stop slippage: the exact path's stop overshoot *is* that slippage. Residual after all three: +0.006 ± 0.004 %-of-slot per trade (hf), within noise, no slack.

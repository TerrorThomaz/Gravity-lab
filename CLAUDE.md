...
- **Dip-bounce with DCA (`scripts/dip_dca_research.py`, 2026-09-26): no edge at either frequency, rejected.** A behavioural capitulation-reversal bet: buy a dip, DCA up to 3 adds, take profit on the bounce with one tranche or exit at break-even once averaged in, stop one step below the last add, 10 slots, maker limits. Two variants, each fixed before its first run:

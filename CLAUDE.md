...
- **Dip-bounce with DCA (`scripts/dip_dca_research.py`, 2026-09-26): no edge, rejected.** A behavioural capitulation-reversal bet. Rules fixed in advance: buy when a coin closes 15% below its 7-day high (top-40 liquid); up to 3 DCA adds every 5%; TP +5% on one tranche, break-even +0.5% once averaged in; stop at P0×0.80 (taker); 10 slots. Fill rules verified against exact sub-hour fill order on random walks: +0.01%-of-slot per trade over 64 seeds. Result (maker): **OosCoins −13.2%/yr (t −1.4), BacktestCoins −7.5%/yr (t −0.8)**, both negative in 2022 and 2024. Win rate 93–94%, but the ~6% of trades that stop out lose several times the book's whole net result. Against the controls: the 15% trigger does pick better entries than random (random-entry control −16.4 / −22.3%/yr), but real prices do **not** bounce more than the same bars shuffled in time (shuffled −9.2 / −8.1%/yr), so there is no reversal effect here to harvest. Without the stop it becomes pure DCA-to-break-even: 97% wins, worst trade −78% of the slot, maxDD −73%, still ≤ +7.5%/yr at t 0.6. That is the martingale tail the stop exists to cap. Removing DCA (one tranche, −5% stop) is far worse: −50%/yr. Crypto only; no stock data in the repo, so the equity version of the behavioural claim is untested here.

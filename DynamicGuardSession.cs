...
// Reacts to ATR expansion and short-term momentum — proactive vs the portfolio-drawdown guard.
...
public class DynamicGuardSession
{
    ...
    public DynamicGuardSession(Candle[] btcH1, DynamicGuardGenotype g)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            // Running sum for avg ATR (O(1) per bar)
            atrSum += atr[i];
            ...
            _mults[i] = g.ComputeMult(atrRatio, mom);
        }
    }
    ...
    // Per-bar true range (unsmoothed; smoothing is done via rolling avg in constructor).

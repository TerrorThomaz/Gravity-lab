...
public class SwingGenotype
{
    ...
    public double AdxThreshold  { get; set; }   // 18–45   uptrend gate

    // ── Entry signal genes ────────────────────────────────────────────────────────
    public int    LookbackCandles  { get; set; }   // 5–40    window to locate the swing high
    ...
    public double TakeProfitAtrMult         { get; set; }   // 1.5–12.0
    ...
    public int    MaxHoldCandles            { get; set; }   // 6–120    4h bars: 6=1d, 42=7d, 120=20d
    ...
    public static SwingGenotype Random(System.Random rng, SwingGenotype? seed = null)
    {
        ...
        return new()
        {
            ...
            AdxThreshold   = Seed(18.0 + rng.NextDouble() * 27.0,         seed?.AdxThreshold ?? 25.0),
            LookbackCandles  = Seed(rng.Next(5, 41),                      seed?.LookbackCandles  ?? 15),
            ...
            StopLossAtrMult           = Seed(0.5 + rng.NextDouble() * 3.5,  seed?.StopLossAtrMult           ?? 1.5),
            TakeProfitAtrMult         = Seed(1.5 + rng.NextDouble() * 10.5, seed?.TakeProfitAtrMult         ?? 4.0),
            TrailingActivationAtrMult = Seed(1.0 + rng.NextDouble() * 7.0,  seed?.TrailingActivationAtrMult ?? 3.0),
            TrailingStopAtrMult       = Seed(0.5 + rng.NextDouble() * 4.5,  seed?.TrailingStopAtrMult       ?? 1.5),
            MaxHoldCandles            = Seed(rng.Next(6, 121),              seed?.MaxHoldCandles            ?? 42),
        };
    }
    ...
    public SwingGenotype Mutate(System.Random rng, double rate)
    {
        ...
        return new SwingGenotype
        {
            ...
            LookbackCandles  = NudgeInt(LookbackCandles,  5, 40, 5),
            ...
            TakeProfitAtrMult         = Nudge(TakeProfitAtrMult,         1.5, 12.0, 1.5),
            ...
            MaxHoldCandles            = NudgeInt(MaxHoldCandles, 6, 120, 12),
        };
    }
    ...
    public SwingGenotype ClampToBounds() => new()
    {
        ...
        LookbackCandles  = Math.Clamp(LookbackCandles,   5,   40),
        ...
        TakeProfitAtrMult         = Math.Clamp(TakeProfitAtrMult,         1.5, 12.0),
        ...
        MaxHoldCandles            = Math.Clamp(MaxHoldCandles,             6,  120),
        Fitness = Fitness,
    };

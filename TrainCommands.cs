...
static class TrainCommands
{
    ...
    public static async Task RunMomLongTrain(BybitRestClient client)
    {
        Console.WriteLine("=== Gravity-gen2 | BULL TRAIN (momentum long, 1h setup + 15m entry/exit, 53 coins, ~3yr) ===\n");

        var trainSyms = new[]
        {
            ("SOLUSDT", 1.0), ("ETHUSDT", 1.0), ("BNBUSDT", 1.0), ("XRPUSDT", 1.0),
            ("DOGEUSDT", 1.0), ("AVAXUSDT", 1.0), ("ADAUSDT", 1.0), ("LINKUSDT", 1.0),
            ("DOTUSDT", 1.0), ("ATOMUSDT", 1.0), ("NEARUSDT", 1.0), ("INJUSDT", 1.0),
            ("OPUSDT", 1.0), ("ARBUSDT", 0.9), ("UNIUSDT", 1.0), ("AAVEUSDT", 1.0),
            ("RUNEUSDT", 1.0), ("STXUSDT", 1.0), ("WIFUSDT", 0.8), ("MEMEUSDT", 0.8),
            ("1000BONKUSDT", 0.9), ("1000PEPEUSDT", 0.9), ("1000FLOKIUSDT", 0.9),
            ("SUIUSDT", 0.9), ("APTUSDT", 0.9), ("LDOUSDT", 1.0), ("TIAUSDT", 0.8),
            ("SEIUSDT", 0.8), ("WLDUSDT", 0.8), ("JUPUSDT", 0.8), ("ONDOUSDT", 0.8),
            ("PYTHUSDT", 0.8), ("GMXUSDT", 0.9), ("SANDUSDT", 1.0), ("MANAUSDT", 1.0),
            ("GALAUSDT", 1.0), ("APEUSDT", 1.0), ("BCHUSDT", 1.0), ("MATICUSDT", 1.0),
            ("TRXUSDT", 1.0), ("XLMUSDT", 1.0), ("ETCUSDT", 1.0), ("ALGOUSDT", 0.9),
            ("HBARUSDT", 0.9), ("ICPUSDT", 0.9), ("MKRUSDT", 0.9), ("CRVUSDT", 0.9),
            ("SNXUSDT", 0.9), ("FETUSDT", 0.9), ("AXSUSDT", 0.9), ("IMXUSDT", 0.9),
            ("1000SHIBUSDT", 0.9), ("ORDIUSDT", 0.8),
        };

        Console.WriteLine($"  Fetching {trainSyms.Length} coins (15m → 1h, ~3yr)...");
        var semBull = new SemaphoreSlim(4);
        var bullFetchTasks = trainSyms.Select(async t =>
        {
            await semBull.WaitAsync();
            try
            {
                var m15 = await CandleFetcher.FetchFifteenMinCandlesCached(client, t.Item1, batches: 113);
                var h1  = FadeShortSimulator.AggregateCandles(m15.ToArray(), 4);
                Console.WriteLine($"  {t.Item1}: {m15.Count} 15m → {h1.Length} h1 (~{h1.Length / 24.0:F0}d)");
                return (t.Item1, t.Item2, h1, m15.ToArray());
            }
            finally { semBull.Release(); }
        });
        var bullFetched = await Task.WhenAll(bullFetchTasks);

        var bullCoins = new List<MomLongGA.CoinData>();
        foreach (var (sym, weight, h1, m15) in bullFetched)
        {
            if (h1.Length < 150) { Console.WriteLine($"  {sym}: skip (insufficient data)"); continue; }
            ...
            int h1Train = (int)(h1.Length * 0.75);
            int h1Val   = (int)(h1.Length * 0.875);
            bullCoins.Add(new MomLongGA.CoinData(
                h1[..h1Train], h1[h1Train..h1Val],
                m15[..(h1Train * 4)], m15[(h1Train * 4)..(h1Val * 4)], weight));
        }
        if (bullCoins.Count == 0) { Console.WriteLine("No data."); return; }

        MomLongGenotype? bullSeed = null;
        if (File.Exists(Config.MomLongGenoFile))
        {
            var candidate = JsonSerializer.Deserialize<MomLongGenotypeDto>(File.ReadAllText(Config.MomLongGenoFile))!.ToGenotype();
            if (candidate.Fitness > 0) { bullSeed = candidate; Console.WriteLine($"  Seeding from {Config.MomLongGenoFile}: {bullSeed}"); }
            else Console.WriteLine("  Skipping seed (fitness ≤ 0 — training from scratch)");
        }

        Console.WriteLine($"\n  Training on {bullCoins.Count} coins\n");
        var bullBest = new MomLongGA(80, 150, verbose: true).Run(bullCoins, bullSeed);
        Console.WriteLine($"\nFrozen genotype:\n  {bullBest}\n");
        File.WriteAllText(Config.MomLongGenoFile, JsonSerializer.Serialize(MomLongGenotypeDto.From(bullBest),
            new JsonSerializerOptions { WriteIndented = true }));
        Console.WriteLine($"  Saved → {Config.MomLongGenoFile}");

        Console.WriteLine("\n─── Overfit check (train 75% vs val 12.5%) ───");
        var bullTRet = bullCoins.SelectMany(cd => MomLongSimulator.GetMomLongReturns(bullBest, cd.TrainH1.Span, cd.TrainM15.Span).Select(t => t.Return)).ToList();
        var bullVRet = bullCoins.SelectMany(cd => MomLongSimulator.GetMomLongReturns(bullBest, cd.ValH1.Span,   cd.ValM15.Span  ).Select(t => t.Return)).ToList();
        int bullTCC  = bullCoins.Sum(cd => cd.TrainH1.Length) * 12;
        int bullVCC  = bullCoins.Sum(cd => cd.ValH1.Length)   * 12;
        CandleFetcher.PrintSplitStats("Train 75%",  bullTRet, bullTCC);
        CandleFetcher.PrintSplitStats("Val  12.5%", bullVRet, bullVCC);
        double bvExp = bullVRet.Count > 0 ? bullVRet.Average() : 0;
        double btExp = bullTRet.Count > 0 ? bullTRet.Average() : 0;
        Console.WriteLine(bvExp < btExp * 0.4 || bvExp <= 0
            ? "\n  !! Possible overfit — val expectancy < 40% of train"
            : "\n  OK — val expectancy within acceptable range");
        Console.WriteLine($"\nNext: dotnet run -- backtest");
    }

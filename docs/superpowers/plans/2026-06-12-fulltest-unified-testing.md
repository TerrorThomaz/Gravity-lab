# Fulltest Unified Testing Suite Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add SwingLong to the standalone OOS backtest, then create a single `fulltest` command that fetches candles once and prints a 7-section condensed master summary (strategy performance val vs OOS, portfolio sims, year-by-year, statistical edge, crash/rally, router impact, forward projection).

**Architecture:** One new file `FullTest.cs` fetches all training + OOS + BTC/ETH candles in a single pass, builds a shared router session, collects val (80/20 split of training coins) and OOS (full history, 28 coins) trade lists for all 5 strategies, applies the concurrent cap filter once to each, then prints 7 condensed sections. No per-coin tables. `OosBacktest.cs` gets SwingLong added as a standalone fix. `Program.cs` gets `case "fulltest"`.

**Tech Stack:** C# .NET 10, existing simulator statics (`FadeShortSimulator`, `GridSimulator`, `FadeLongSimulator`, `DipLongSimulator`, `SwingLongSimulator`), `StrategyStats`, `CrashAnalyser`, `PortfolioReplay`, `Simulator`, `RegimeRouterSession`.

---

## File Structure

- **Modify:** `OosBacktest.cs` — add SwingLong load, simulation block, summary line, per-coin breakdown, OosStrategyHold case
- **Create:** `FullTest.cs` — single candle fetch + 7-section condensed report
- **Modify:** `Program.cs` — add `case "fulltest"`

---

## Task 1: Add SwingLong to OosBacktest.cs

**Files:**
- Modify: `OosBacktest.cs`

Context: `RunOosBacktest` currently loads FadeShort, Grid, FadeLong, DipLong but not SwingLong. The OOS summary misses it entirely. SwingLong uses `RegimeRouterGA.StrategyKind.DipLong` for its router gate (same bull-regime gate as DipLong). SwingLong's strategy key string is `"swing_long"`. `GetSwingLongReturns` returns `List<(DateTime Time, double Return, string Kind)>` — no `RegimeBarsActive` field. The concurrent cap switch already has a `"swing_long"` case (added by PortfolioReplay in a prior task). `OosStrategyHold` is a local static function that needs a `SwingLongGenotype? slG` parameter added.

- [ ] **Step 1: Add SwingLong genotype load (after dlG line)**

In `OosBacktest.cs`, after:
```csharp
DipLongGenotype?      dlG     = File.Exists(Config.DipLongGenoFile)   ? ... : null;
```
Add:
```csharp
...
And after the `if (dlG != null) Console.WriteLine(...)` block add:
```csharp
if (slG     != null) Console.WriteLine($"SwingLong: {slG}");
else                 Console.WriteLine("SwingLong: not found — skipping");
```

- [ ] **Step 2: Add slTrades and slCoinStats lists**

After `var dlTrades = new List<(DateTime Time, double Return, double Conf)>();` add:
```csharp
var slTrades    = new List<(DateTime Time, double Return, double Conf)>();
```

After `var dlCoinStats = new List<...>();` add:
```csharp
...
- [ ] **Step 3: Add SwingLong simulation block (after the DipLong `}` closing brace, before `if (allTrades.Count == 0)`)**
...
- [ ] **Step 4: Sort slTrades (after `dlTrades.Sort` line)**

```csharp
slTrades.Sort((a, b) => a.Time.CompareTo(b.Time));
```

- [ ] **Step 5: Add slRet and summary line**

After `var dlRet = dlTrades.Select(t => t.Return).ToList();` add:
```csharp
var slRet = slTrades.Select(t => t.Return).ToList();
```

After `if (dlRet.Count > 0) Console.WriteLine($"  DipLong: ...")` add:
```csharp
if (slRet.Count > 0)
    Console.WriteLine($"  SwingLong: {slRet.Count,4} trades  PF={Simulator.ProfitFactor(slRet):F2}  {Pct(slRet)}");
```

- [ ] **Step 6: Update OosStrategyHold to include slG**
...
And update the call site (the `allTradesForExposure` line) to pass `slG` as the last argument:
```csharp
...
- [ ] **Step 7: Add SwingLong per-coin breakdown (at end, after dlCoinStats breakdown block)**
...
```bash
dotnet run -c Release -- oosbacktest 2>&1 | grep -E "SwingLong|SWINGLONG|OOS COMBINED"
```
Expected: output includes `══ SWINGLONG` section and `SwingLong:` line in summary.

- [ ] **Step 10: Commit**

```bash
git add OosBacktest.cs
git commit -m "feat: add SwingLong to oosbacktest"
```

---

## Task 2: Create FullTest.cs

**Files:**
- Create: `FullTest.cs`

Context: This is the main file. It fetches all coins once, collects val trades (training coins, 80/20 split) and OOS trades (28 OOS coins, full history) for all 5 strategies, applies concurrent cap to each trade list separately, then prints 7 sections.

Key types and methods used:
- `FadeShortSimulator.GetFadeShortReturns(g, h1, m15)` → `List<(DateTime Time, double Return, string _)>`
- `GridSimulator.GetGridReturns(g, h1)` → `List<(DateTime Time, double Return, string _)>`
- `FadeLongSimulator.GetFadeLongReturns(g, h1, m15)` → `List<(DateTime Time, double Return, string Kind, int RegimeBarsActive)>`
- `DipLongSimulator.GetDipLongReturns(g, h1, m15)` → `List<(DateTime Time, double Return, string Kind, int RegimeBarsActive)>`
- `SwingLongSimulator.GetSwingLongReturns(g, h1, m15)` → `List<(DateTime Time, double Return, string Kind)>`
- `session.IsActive(RegimeRouterGA.StrategyKind.Grid/FadeLong/DipLong, time)` for router gating
- SwingLong gate = `RegimeRouterGA.StrategyKind.DipLong` (bull regime)
- `PortfolioReplay.FilterByConcurrentCap(List<PortfolioReplay.Trade>)` → `List<PortfolioReplay.Trade>` (`.EntryTime`, `.Return`, `.Conf`, `.Strategy`)
- `Simulator.SimulatePortfolioExposureCapped(trades, maxTotalExposurePct, maxPositionFrac?)` where trades = `List<(DateTime, double Return, double Conf, TimeSpan Hold)>`
- `StrategyStats.OneSampleT(rets)` → `.Mean`, `.T`, `.PValue`, `.Sig95`, `.Sig99`
- `StrategyStats.Bootstrap(rets)` → `.ProbPositive`
- `StrategyStats.DeflatedSharpe(rets, candleCount)`
- `StrategyStats.Compare(nameA, listA, nameB, listB, candleCount)` — prints comparison block
- `CrashAnalyser.DetectCrashes(btcH1)`, `.Report(crashes, crashTrades)`, `.DetectRallies(btcH1)`, `.ReportRallies(rallies, crashTrades)`, `.SyntheticWorstCase(crashTrades)`
- `crashTrades` type: `List<(DateTime Open, DateTime Close, double Return, double HalfKelly, string Strategy)>`
- `Config.BacktestCoins`, `Config.OosCoins`, `Config.MaxTotalExposurePct` (0.30), `Config.MinMedianVolUsdM` (0.5)
- Val split: `int h1Split = (int)(h1.Length * 0.8); int m15Split = h1Split * 4; var h1Val = h1[h1Split..]; var m15Val = m15[Math.Min(m15Split, m15.Length)..];`
- OOS vol filter: `const double oosMinVol = 0.05;` (lower than training coins)
- FadeShort screen: use `h1Train` if `h1Train.Length >= 4380`, else fall back to full `h1` (same as CombinedBacktest)
- DipLong/FadeLong/SwingLong: only process if `h1Val.Length >= 100 && m15Val.Length >= 400` for val, `h1.Length >= 300 && m15.Length >= 1200` for OOS

- [ ] **Step 1: Create FullTest.cs with complete implementation**

Create `/home/thomas/Gravity-gen2/FullTest.cs`:
...
static class FullTest
{
    public static async Task RunFullTest(BybitRestClient client)
    {
        ...
        var nrFiltered = PortfolioReplay.FilterByConcurrentCap(nrCapIn)
        ...
        Console.WriteLine($"  {"Grid",-12}  {valAll.Count(t=>t.Strategy=="grid"),11}  {nr.Count(t=>t.Strategy=="grid"),9}  {nr.Count(t=>t.Strategy=="grid")-valAll.Count(t=>t.Strategy=="grid"),+6}");
        Console.WriteLine($"  {"DipLong",-12}  {valAll.Count(t=>t.Strategy=="diplong"),11}  {nr.Count(t=>t.Strategy=="diplong"),9}  {nr.Count(t=>t.Strategy=="diplong")-valAll.Count(t=>t.Strategy=="diplong"),+6}");
        Console.WriteLine($"  {"FadeLong",-12}  {valAll.Count(t=>t.Strategy=="fadelong"),11}  {nr.Count(t=>t.Strategy=="fadelong"),9}  {nr.Count(t=>t.Strategy=="fadelong")-valAll.Count(t=>t.Strategy=="fadelong"),+6}");
        Console.WriteLine($"  {"SwingLong",-12}  {valAll.Count(t=>t.Strategy=="swing_long"),11}  {nr.Count(t=>t.Strategy=="swing_long"),9}  {nr.Count(t=>t.Strategy=="swing_long")-valAll.Count(t=>t.Strategy=="swing_long"),+6}");
...
If there are errors, fix them before continuing. Common pitfall: the `valSim` list uses `.Item1` for DateTime access since the tuple fields are anonymous in lambdas — this is correct C# syntax for `ValueTuple` when accessed through a generic list.

- [ ] **Step 3: Commit**

```bash
git add FullTest.cs
git commit -m "feat: add FullTest.cs unified testing suite (7 sections)"
```

---

## Task 3: Wire Program.cs and run integration test

**Files:**
- Modify: `Program.cs`

- [ ] **Step 1: Add fulltest case to Program.cs**

In `Program.cs`, add after the `oosbacktest` case:
```csharp
case "fulltest":        await FullTest.RunFullTest(client);                                        break;
```

And in the default help block, add:
```csharp
Console.WriteLine("  dotnet run -- fulltest           Full test suite: val + OOS + stats + crash/rally (condensed)");
```
...
```bash
dotnet run -c Release -- fulltest 2>&1 | grep -E "═══|STRATEGY|PORTFOLIO|YEAR|STATISTICAL|CRASH|ROUTER|FORWARD|error|Error" | head -30
```
Expected: output includes all 7 section headers (`STRATEGY PERFORMANCE`, `PORTFOLIO SIMULATION`, `YEAR-BY-YEAR`, `STATISTICAL EDGE`, `CRASH / RALLY`, `ROUTER IMPACT`, `FORWARD PROJECTION`), no `Error` lines.

- [ ] **Step 4: Full run and sanity check**

```bash
dotnet run -c Release -- fulltest 2>&1
```
Verify:
- Strategy performance table has 5 rows (FadeShort, Grid, DipLong, SwingLong, FadeLong) with both val and OOS columns populated
- Portfolio sim table has 4 columns (Val 5%cap, Val Kelly, OOS 5%cap, OOS Kelly) all showing non-zero returns
- Year-by-year shows at least 3 years
- Statistical edge shows t-stats and p-values for strategies with ≥10 trades
- Crash/rally section prints crash events
- Router impact shows trade counts with and without router
- Forward projection shows 90d slice

- [ ] **Step 5: Commit**

```bash
git add Program.cs
git commit -m "feat: wire fulltest command in Program.cs"
```

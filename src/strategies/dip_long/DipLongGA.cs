...
public class DipLongGA
{
    ...
    private double Fitness(DipLongGenotype ind, IReadOnlyList<CoinData> coins, bool useValidation, int folds = 5)
    {
        ...
        // Avoids the minLen bottleneck where one short-history coin (e.g. APTUSDT with
        // only 1908 training bars) would cap all other coins at that length, making every
        // fold too narrow to accumulate MinTradesPerFold regime-valid trades.
        int k = folds;
